...
math: true
mermaid: false
img_path: /assets/images/20230109BS4
---
...
如果 tag 只有一个 `NavigableString` 类型子节点，那么这个tag可以使用 `.string` 得到子节点。如果一个tag仅有一个子节点，那么这个tag也可以使用 `.string` 方法，输出结果与当前唯一子节点的 `.string` 结果相同。
